...
| `comfy_org_api_key` | `string` | `None` | No | Optional per-request Comfy.org API key for API Nodes. Overrides the `COMFY_ORG_API_KEY` environment variable if both are set. |
...
- ✅ **Image URL Support**: Automatically downloads and converts HTTP(S) URLs to Base64
- ✅ **Path Normalization**: Automatically converts Windows-style paths (`\\`) to Unix-style (`/`)
- ✅ **Video Output Support**: Supports both image and video outputs (MP4, WebM, etc.)
- ✅ **S3 Integration**: Optional S3 upload for generated images/videos
- ✅ **WebSocket Reconnection**: Robust error handling with automatic reconnection
...
For more information, see the main [README.md](https://github.com/your-username/runpod-comfyui-cuda128-pure) in the repository.
